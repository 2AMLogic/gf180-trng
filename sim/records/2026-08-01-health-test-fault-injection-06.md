---
record: 2026-08-01-health-test-fault-injection-06
...
analysis:
  ...
  tstop: n/a (cycle-count driven: 2335 sampler clocks)
  tstep: n/a
  tmax: n/a
  ...
  runs: 1
seeds: [30]   # SHA-256 counter-mode source, bit-identical on any platform
...
raw:
  path: sim/records/raw/2026-08-01-health-test-fault-injection-06/
  files:
    - summary.json  sha256:b24b551b343323adc6faded93597b09e5b800c53b218d2d7e8ad76375c983fdb
wall_time: 0.4s
---

## Result

Scenario `startup-gate` -- DR-0002 Sec.4: an RCT/APT failure must reset the start-up counter, and startup_req must discard any in-flight window and re-arm it -- ht_startup_pass must only ever pulse after a full STARTUP_SAMPLES run with neither test failing.

| Quantity | Value |
|---|---|
| C_RCT / C_APT / W | 81 / 824 / 1024 |
| clean W-sample window passes start-up | True |
| mid-window RCT failure resets the start-up counter | True |
| startup_req discards the in-flight window | True |
| a fresh full window after restart passes | True |
...
```sh
python3 sim/tb/health-test-fault-injection/run_demo.py --scenario startup-gate --no-write
```
