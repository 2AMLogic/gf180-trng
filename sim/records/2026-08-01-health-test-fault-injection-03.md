---
record: 2026-08-01-health-test-fault-injection-03
date: 2026-08-01T23:07:41Z
status: valid
...
analysis:
  ...
  tstop: n/a (cycle-count driven: 2300 sampler clocks)
  tstep: n/a
  tmax: n/a
...
raw:
  path: sim/records/raw/2026-08-01-health-test-fault-injection-03/
  files:
    - summary.json  sha256:35da3cc17e23ec1d67da0d9a7ce8c1b2e1fd7bb6b49d4d2dc172560511eaae93
    - raw_bits.bin  sha256:5f77a51fd7aedf245fb7ecca7659a4e8e182d1cb3ce68a16d19d9b76f48df1b5
wall_time: 0.1s
---

## Result

Scenario `stuck-output` -- DR-0002's first detection-latency target: a stuck raw output must be detected within C_RCT samples of onset.

| Quantity | Value |
|---|---|
| lead-in + fault bits total | 2300 |
| fault onset (sample index) | 2000 |
| fault generator | `constant_bits` |
| C_RCT / C_APT / W | 81 / 824 / 1024 |
| first event index after onset | 2078 |
| detection latency (samples after onset) | 78 |
| DR-0002 latency bound | c_rct - 1 = 80 |
...
```sh
python3 sim/tb/health-test-fault-injection/run_demo.py --scenario stuck-output --no-write
```
