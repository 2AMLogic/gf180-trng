...
level, the DR-0001 raw tap) feeding conditioner + health tests + interface
...
in RTL, gluing together the three real synthesisable modules
(``crc32_conditioner.v``, ``rct_apt.v``, ``trng_interface.v``) with plain
wires and no extra logic; ``sim/tests/test_trng_top.py`` checks that this
...
here: :class:`TopLevel` takes ``raw_bit``/``raw_valid`` per cycle from
whatever the caller supplies, exactly as ``crc32_conditioner.Conditioner``
and ``rct_apt.HealthTest`` already do individually.
...
for the whole cycle *after* the edge that computed them. ``trng_interface.v``'s
...
1. ``iface.peek_control(ht_fail_rct=<last>, ht_fail_apt=<last>, **bus)`` --
   the interface's purely-combinational response to the *previous* edge's
   latched health-test flags plus this cycle's register-bus stimulus.
...
@dataclass
class TopLevel:
    """The three digital blocks, wired as ``design/xschem/trng_top.sch``'s
    ...
    clock, fixed external, is what all three blocks and the raw tap share).
    """
    ...
    #: The health test's registered failure flags, as the interface would
    #: see them at the start of this cycle -- see the module docstring.
    ...
    def step(
        ...
        **bus,
        ...
        """Advance one sampler-clock edge. ``bus`` carries the register-bus
        stimulus (``reg_sel``/``reg_write``/``reg_addr``/``reg_wdata``/
        ``str_ready``) that ``trng_interface.Interface.step`` accepts.
        Returns the interface's :class:`~trng_interface.Outputs` for this
        cycle -- the full top-level output (register reads, the streaming
        port, ``ht_alarm``)."""
        self.cycles += 1
        if raw_valid:
            self.raw_samples += 1
