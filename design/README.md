...
| [`sim/tb/array-liveness-tap-phase-{clocked,static}/`](../sim/tb/array-liveness-tap-phase-clocked/) | the same `clk`-locked question asked on the array as it SHIPS rather than on an isolated ring: two `ro_ring11` rings, both `ro_buf` output buffers, `xa1` on both buffer outputs and all four digitizers on those same nodes, with `clk` running against the identical netlist with `clk` parked — the residual #76 could only bound (#87, **not yet run**) |
| [`sim/tb/array-liveness-tap-phase-xsb-{clocked,static}/`](../sim/tb/array-liveness-tap-phase-xsb-clocked/) | the same shipped array with `xsr1`/`xsr2` removed, leaving the DR-0001 raw-tap digitizer `xsb` on the combiner output `xo` as the only `clk`-driven load with a path to a ring: what the `xo` path alone delivers to a ring node (#87, **not yet run**) |
