...
| Power | < 500 µW active, binding at `ff` / +10 % supply (fastest RO — max measured `f_osc` 2.30 GHz at −40 °C); < 1 µA idle, binding at `ff` / +10 % / +125 °C (max leakage). **Now evidenced (#14, re-measured after #78's buffer adoption, #174/[DR-0023]'s substitution of #145's measured gate-level digital figure for the pre-synthesis estimate, #255/#266's re-synthesis and re-place-and-route of the digital section at [DR-0020]'s ratified `FIFO_DEPTH = 2`, and #264/#277's re-tune of that depth-2 build's `max_transition_ns` (8.0 → 4.0) and rebuild, with #145's fifteen-corner `level: gate` sweep re-run against each rebuild — the current family is `sim/records/2026-09-22-digital-sta-power-{01..15}.md`): active 755.6 µW — missed by 1.5×, at 151.1 % of the row. Idle 2.12 µA — missed by ~2.1×**, and the cause on both halves is still the synthesized-and-placed digital section (345.6 µW active / 7.515 µW — 2.087 µA — leakage, MEASURED-at-gate-level, down from 712.4 µW / 4.062 µA at the depth-8 build #174/[DR-0023] first measured), not the analog block (393.2 + 16.9 µW active / 32.8 nA idle, measured, unchanged by the digital section's depth). Neither miss is absorbed: see [`sim/characterization-startup-and-power-budget.md`](sim/characterization-startup-and-power-budget.md), §0a of [`sim/characterization-digital-sta-area-power.md`](sim/characterization-digital-sta-area-power.md), [DR-0023] (`Accepted` 2026-09-07 via the two-key ratification mechanism, #213 — the active miss and the idle figure's revision, both stated there against the depth-8 build; `sim/tools/power_rollup.py` reads the digital term live from the committed record family, so #255/#266's depth-2 re-run and #264/#277's re-tuned rebuild update the tool's own verdict without a new decision record) and [DR-0017] (`Proposed`, the idle row's own diagnosis and options, unsuperseded — not part of the #213 ratification batch), and the note below | — |
| Area | < 0.05 mm² | — |
...
>   topology, rebuilt, and re-ran the same sweep once more** — the clean build
>   is the committed
>   `sim/records/2026-09-22-digital-sta-power-{01..15}.md` family, and
>   `power_rollup.py` picks each of these up automatically because its digital
>   term already reads live from that family ([DR-0023], §0a of
...
>   - **Active: missed, by 1.5×** (was 2.2×). 755.6 µW at `ff`/−40 °C/3.63 V —
>     entropy source 393 µW (measured), sampler 16.9 µW (measured), digital
>     section 345.6 µW (**MEASURED-at-gate-level** at its own worst corner,
>     `ff_125C_3v60`/`rc-max`, [DR-0021]; down from 358.5 µW at #255/#266's
...
>     section alone is now 3.2× everything the array leaves, versus 6.7× at
...
>     re-synthesis (768.5 µW, 153.7 %) is the reason the miss narrows, and
>     #264/#277's re-tuned rebuild is the number above. The
...
>     moved.
>   - **Idle: missed, by ~2.1×** (was ~2.0× at the violating depth-2 build,
>     ~4.1× at depth 8, 4.5× estimated). 2.12 µA at
>     `ff`/+125 °C/3.63 V — analog block 32.8 nA (3.3 % of the row, measured
>     across 45 corners, unchanged), digital leakage 2.087 µA / 7.515 µW
>     (**MEASURED-at-gate-level**, down from 4.062 µA at the depth-8 build and
